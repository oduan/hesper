      ...
      if (event.type === 'message.completed' && event.message.role === 'assistant' && event.message.runId) {
        ...
        if (!completedRun?.parentRunId) {
          ...
          const source = session && isDefaultSessionTitle(session.title)
            ...
            : undefined
          const sessionEffectiveDefaultModelId = session?.defaultModelId?.trim() || undefined
          const modelId = runModelIdsRef.current[event.message.runId] ?? sessionEffectiveDefaultModelId ?? defaultFallbackModelId
