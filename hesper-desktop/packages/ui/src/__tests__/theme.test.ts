...
describe('built-in theme registry', () => {
  ...
  it('exports semantic theme tokens and keeps darkTheme as a compatibility alias', () => {
    ...
    expect(themeTokens.color.accentContrast).toBe('var(--hesper-color-accent-contrast, #16161e)')
    expect(themeTokens.color.previewBackground).toBe('var(--hesper-color-preview-background, #16161e)')
    expect(themeTokens.color.scrollbarThumbActive).toBe('var(--hesper-color-scrollbar-thumb-active, rgba(192, 202, 245, 0.38))')
  })
})
