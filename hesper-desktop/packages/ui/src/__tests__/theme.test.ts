...
describe('built-in theme registry', () => {
  ...
  it('generates Catppuccin Latte CSS variables with a neutral gray accent', () => {
    ...
    expect(variables.colorScheme).toBe('light')
    expect(variables['--hesper-font-size']).toBe('14px')
    expect(variables['--hesper-color-background']).toBe('#e6e9ef')
    expect(variables['--hesper-color-surface']).toBe('#eff1f5')
    expect(variables['--hesper-color-surface-muted']).toBe('#eceef4')
    expect(variables['--hesper-color-text']).toBe('#4c4f69')
    expect(variables['--hesper-color-text-muted']).toBe('#6c6f85')
    expect(variables['--hesper-color-border']).toBe('#d8dce6')
    expect(variables['--hesper-color-border-subtle']).toBe('rgba(216, 220, 230, 0.38)')
    ...
    expect(variables['--hesper-color-hover']).toBe('rgba(188, 192, 204, 0.08)')
    expect(variables['--hesper-color-danger-soft']).toBe('rgba(210, 15, 57, 0.14)')
    expect(variables['--hesper-color-shadow']).toBe('rgba(76, 79, 105, 0.12)')
  ...
  it('generates Catppuccin Mocha CSS variables with a neutral gray accent', () => {
    ...
    expect(variables['--hesper-color-background']).toBe('#181825')
    expect(variables['--hesper-color-surface']).toBe('#1e1e2e')
    expect(variables['--hesper-color-surface-muted']).toBe('#1b1b2a')
    expect(variables['--hesper-color-text']).toBe('#cdd6f4')
    expect(variables['--hesper-color-text-muted']).toBe('#a6adc8')
    expect(variables['--hesper-color-border']).toBe('#2a2b3d')
    expect(variables['--hesper-color-border-subtle']).toBe('rgba(42, 43, 61, 0.38)')
    expect(variables['--hesper-color-accent']).not.toBe('#cba6f7')
    expect(variables['--hesper-color-accent']).toBe('#a6adc8')
    expect(variables['--hesper-color-accent-contrast']).toBe('#181825')
    expect(variables['--hesper-color-hover']).toBe('rgba(166, 173, 200, 0.10)')
    expect(variables['--hesper-color-success-soft']).toBe('rgba(166, 227, 161, 0.16)')
    expect(variables['--hesper-color-preview-background']).toBe('#181825')
    expect(variables['--hesper-color-code-background']).toBe('#181825')
  })

  it('generates Dracula CSS variables with softer dark surfaces', () => {
    const variables = vars(createThemeVariables({ themeId: 'dracula', mode: 'dark', fontSize: 14 }))

    expect(variables.colorScheme).toBe('dark')
    expect(variables['--hesper-color-background']).toBe('#282a36')
    expect(variables['--hesper-color-surface']).toBe('#2f3140')
    expect(variables['--hesper-color-surface-muted']).toBe('#343746')
    expect(variables['--hesper-color-border']).toBe('#3a3d4d')
    expect(variables['--hesper-color-border-subtle']).toBe('rgba(58, 61, 77, 0.38)')
    expect(variables['--hesper-color-accent']).toBe('#bd93f9')
    expect(variables['--hesper-color-hover']).toBe('rgba(189, 147, 249, 0.10)')
    expect(variables['--hesper-color-code-background']).toBe('#282a36')
  })

  it('generates Tokyo Night CSS variables with softer dark surfaces', () => {
    const variables = vars(createThemeVariables({ themeId: 'tokyo-night', mode: 'dark', fontSize: 14 }))

    expect(variables.colorScheme).toBe('dark')
    expect(variables['--hesper-color-background']).toBe('#1a1b26')
    expect(variables['--hesper-color-surface']).toBe('#1e2130')
    expect(variables['--hesper-color-surface-muted']).toBe('#202436')
    expect(variables['--hesper-color-border']).toBe('#2b3046')
    expect(variables['--hesper-color-border-subtle']).toBe('rgba(43, 48, 70, 0.38)')
    expect(variables['--hesper-color-accent']).toBe('#7aa2f7')
    expect(variables['--hesper-color-hover']).toBe('rgba(122, 162, 247, 0.10)')
    expect(variables['--hesper-color-code-background']).toBe('#1a1b26')
  })
